  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        "Inside selfTests, use only the framework DSL: no JavaScript, HTML, selectors, URLs, executable expressions, or free-form failure messages. Test only behavior explicitly stated in SKILL.md and assert observable outcomes. For reachability through an overlay, assert hit-test on the intended underlying target; never require the overlay's pointerEvents, visibility, removal, or stacking style to equal one implementation unless SKILL.md explicitly mandates that exact property. When SKILL.md requires dynamically inserted overlays, put the underlying target in the initial fixture, add the covering overlay later with append-node, wait for observation, and then hit-test the underlying target.",
        "Overlay repair must be viewport-independent. If the specification includes targets below the initial viewport or overlays inserted while offscreen, do not rely only on elementFromPoint/elementsFromPoint at mutation time; use geometry-based overlap detection or reliably rescan on viewport entry/scroll. Model this in public selfTests with scroll-page before the final hit-test.",
        ...
        "For paste behavior, preserve the browser's native paste default action: the paste-text workflow must report event-default-prevented false and the final value must contain the inserted text. Do not call preventDefault on paste/beforeinput or replace the native edit. If a page-world input handler rolls the completed native edit back, a later checkpoint may reapply that event's InputEvent.data with native editing APIs, without reading clipboard contents or the control's existing value.",
  ...
  return [
    { role: "system", content: testerInstructions },
    {
      ...
      content: [
        ...
        "For every paste blocker family, assert that paste-text reports event-default-prevented false as well as the correct final value. The candidate must preserve the browser's native paste default action rather than canceling it and manually inserting text.",
        ...
        "For keyboard copy or cut behavior, use copy-shortcut; never substitute a lone copy/cut event or hand-written keydown sequence. Assert its event-default-prevented result and final target value. Never add a copy/cut flag-only observer or require its call count: generated protection may isolate page handlers while the trusted default operation succeeds. The trusted runner performs the native cut edit when the default path remains un-cancelled, so do not require generated code to mutate the field value itself.",
        "When SKILL.md names DOM event-handler properties or return-false cancellation, model them with registration property and effect return-false; do not treat listener preventDefault coverage as equivalent.",
        "Assert final observable outcomes for effectful blockers. Do not require their call count to be zero; use a flag-only blocker only when SKILL.md explicitly requires the handler itself not to run. For reachability through an overlay, assert hit-test on the intended underlying target; never require the overlay's pointerEvents, visibility, removal, or stacking style to equal one implementation unless SKILL.md explicitly mandates that exact property. When SKILL.md requires dynamically inserted overlays, put the underlying target in the initial fixture, add the covering overlay later with append-node, wait for observation, and then hit-test the underlying target.",
        "When the specification requires viewport-independent overlay repair, include a case whose target and later-appended overlay begin below the initial viewport, then use scroll-page to bring the target into view before the final hit-test. This must catch implementations that call elementsFromPoint only once while the overlay is offscreen.",
        "For textarea fixtures, put the initial editable value in node text (the runner also normalizes an attributes value fallback into the live property). When selected page text is followed by a real click into an ordinary control, use click-control and verify the stale selection is not restored, focus stays on the control, and ordinary input/click behavior remains usable.",

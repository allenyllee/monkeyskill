...
export async function runFixtureAgent(request) {
  ...
  return chatCompletion(request.model, JSON.stringify({
    schemaVersion: 1,
    summary: `Generated by the generic local fixture agent for ${skill.name}.`,
    modes,
    selfTests: testSpec
  }));
}
...
function makeFixtureTestSpec(skill, criteria) {
  if (criteria.length === 0) throw httpError(422, "Agent request does not contain MSkill criteria.");
  return {
    schemaVersion: 1,
    tests: criteria.map((criterion, index) => {
      const capability = criterion.startsWith("no-") ? criterion.slice(3) : null;
      if (capability && skill.forbiddenCapabilities.includes(capability)) {
        return { id: `fixture-${index}`, kind: "policy", criterion, capability, expected: "denied" };
      }
      return {
        id: `fixture-${index}`,
        kind: "behavior",
        ...
        mode: skill.modes[0],
        fixture: {
          nodes: [{ id: "target", tag: "p", parent: null, text: criterion, attributes: {}, styles: {} }],
          rules: []
        },
        blockers: [],
        steps: [],
        assertions: [{ type: "text-content", target: "target", operator: "eq", value: criterion }]
      };
    })
  };
}

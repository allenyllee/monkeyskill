...
export async function runFixtureAgent(request, options = {}) {
  ...
  if (isTesterRequest(request.messages)) {
    const testSpec = await readFile(join(
      options.projectRoot || projectRoot,
      "scripts",
      "fixtures",
      "restore-right-click.testspec.json"
    ), "utf8");
    return chatCompletion(request.model, testSpec);
  }
  const artifact = await loadPackagedArtifact(options.projectRoot || projectRoot);
  const selfTests = JSON.parse(await readFile(join(
    options.projectRoot || projectRoot,
    "scripts",
    "fixtures",
    "restore-right-click.testspec.json"

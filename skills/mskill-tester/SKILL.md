...
- `{"action":"mutation-burst","target":"feed","count":200,"batchSize":10}`; append bounded batches of ID-bearing nodes with text descendants, then measure through a trusted DOM-quiet checkpoint. Candidate work queued by its observers is part of the duration, so a self-triggering repair loop cannot pass merely because the append task returned. Use this only when the MSkill explicitly requires responsiveness under sustained dynamic DOM changes. `count` is 20–500, `batchSize` is 1–50, and `count` must be divisible by `batchSize`.
- `{"action":"scroll-stress","target":"feed","count":1200,"iterations":10}`; create a bounded large-page fixture containing paired controls and empty overlay candidates, include candidate setup work through a trusted DOM-quiet checkpoint, run repeated scroll frames, then wait for scroll-triggered mutation work to become quiet. Use this only when the MSkill explicitly requires scrolling to remain responsive on a large dynamic page. `count` is 100–2000 and `iterations` is 2–20. Use a scale large enough to expose full-index rebuilds when the criterion requires cost to remain independent of total document size.
- `{"action":"startup-stress","target":"feed","count":1200}`; before candidate installation, create a bounded pre-existing large-page fixture containing paired controls and empty overlay candidates, then measure candidate injection and queued initialization work through a trusted document-wide DOM-quiet checkpoint. This action must be the first step and may appear only once in a test. Use it only when the MSkill explicitly requires responsiveness when injected into an already-large page. `count` is 100–2000.

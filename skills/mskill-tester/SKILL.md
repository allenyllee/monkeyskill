...
- `step-duration`: fields `step`, `operator` (`lt` or `lte`), and integer `value` in milliseconds from 50 through 4000. Use only with an explicitly required responsiveness criterion and a `mutation-burst` or `scroll-stress` step; keep thresholds generous enough to avoid treating ordinary runner variance as a product failure.

...
- `{"action":"scroll-stress","target":"feed","count":300,"iterations":10}`; create a bounded large-page fixture containing paired controls and empty overlay candidates, settle dynamic observers, then measure repeated scroll frames. Use this only when the MSkill explicitly requires scrolling to remain responsive on a large dynamic page. `count` is 100–500 and `iterations` is 2–20.

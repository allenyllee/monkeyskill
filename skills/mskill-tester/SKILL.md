...
- `{"action":"scroll-stress","target":"feed","count":300,"iterations":10}`; create a bounded large-page fixture containing paired controls and empty overlay candidates, include candidate setup work through a trusted DOM-quiet checkpoint, run repeated scroll frames, then wait for scroll-triggered mutation work to become quiet. Use this only when the MSkill explicitly requires scrolling to remain responsive on a large dynamic page. `count` is 100–500 and `iterations` is 2–20.

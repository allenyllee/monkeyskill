(() => {
  const marker = "__monkeySkillRestoreRightClickStandardV111";
  if (window[marker]) return;
  window[marker] = true;

  const protectedEvents = ["contextmenu", "copy", "cut", "selectstart", "dragstart"];
  const handlerAttributes = protectedEvents.map(type => `on${type}`);
  const pointerHandlerAttributes = ["onmousedown", "onpointerdown"];
  const letBrowserHandleIt = event => event.stopImmediatePropagation();

  for (const type of protectedEvents) window.addEventListener(type, letBrowserHandleIt, true);
  ...
  function cleanTree(root) {
    ...
    const selector = [...handlerAttributes, ...pointerHandlerAttributes]
  ...
  observer.observe(document, {
    ...
    childList: true,
    attributes: true,
    attributeFilter: [...handlerAttributes, ...pointerHandlerAttributes]
  });
})();
